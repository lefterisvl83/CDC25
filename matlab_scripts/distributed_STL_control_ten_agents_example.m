...
T1 = Polyhedron([1.5 3.75;-0.75 1.5;1.5 -0.75;3.75 1.5]); T1=plus(T1,[.75;0.75]);
G1 = Polyhedron([-0.5 7.5;2.5 7.5;2.5 10.5;-0.5 10.5]); G1=plus(G1,[.5;10]);
...
% cost function
Lu = [];
%% distrubance sequences for each agent and disturbance feedback input sequences
Mu = [0; 0];           % Mean vector (zero mean)
Sigma = 0.05 * eye(2); % Covariance matrix (0.05 * I_2)
for i=1:10
    wk{i} = mvnrnd(Mu, Sigma, N)';
    vk{i}=zeros(2,100);
end
for i=1:10
    for k=2:100
        vk{i}(:,k)=vk{i}(:,k-1)/2-0.5*eye(2)*wk{i}(:,k-1);
    end
end
%
% Matrix collecting who optimizes at each time k
cO=[ones(10,1) zeros(10,N-1)];
% start iterations
tic;
for k=1:iter
    k
    if k==1
        [uk1{k}, xk1{k}, elapsedTime1{k}] = compute_10_agents_STL_three_edges(zeros(2,N),k,x01,zeros(2,N+1),C1,cc1,rob(1),zeros(2,N+1),zeros(2,N+1),zeros(2,N+1),eg,eg,eg,M,A,B,ulim(1),t1,g1,j1,N,gamma,gurobi_iter,E(end),Xtight);
        [uk2{k}, xk2{k}, elapsedTime2{k}] = compute_10_agents_STL_two_edges(zeros(2,N),k,x02,zeros(2,N+1),C2,cc2,rob(2),zeros(2,N+1),zeros(2,N+1),eg,eg,M,A,B,ulim(2),t2,g2,j2,N,gamma,gurobi_iter,E(end),Xtight);
        [uk3{k}, xk3{k}, elapsedTime3{k}] = compute_10_agents_STL_three_edges(zeros(2,N),k,x03,zeros(2,N+1),C3,cc3,rob(3),zeros(2,N+1),zeros(2,N+1),zeros(2,N+1),eg,eg,eg,M,A,B,ulim(3),t3,g3,j3,N,gamma,gurobi_iter,E(end),Xtight);
        [uk4{k}, xk4{k}, elapsedTime4{k}] = compute_10_agents_STL_three_edges(zeros(2,N),k,x04,zeros(2,N+1),C4,cc4,rob(4),zeros(2,N+1),zeros(2,N+1),zeros(2,N+1),eg,eg,eg,M,A,B,ulim(4),t4,g4,j4,N,gamma,gurobi_iter,E(end),Xtight);
        [uk5{k}, xk5{k}, elapsedTime5{k}] = compute_10_agents_STL_three_edges(zeros(2,N),k,x05,zeros(2,N+1),C5,cc5,rob(5),zeros(2,N+1),zeros(2,N+1),zeros(2,N+1),eg,eg,eg,M,A,B,ulim(5),t5,g5,j5,N,gamma,gurobi_iter,E(end),Xtight);
        [uk6{k}, xk6{k}, elapsedTime6{k}] = compute_10_agents_STL_three_edges(zeros(2,N),k,x06,zeros(2,N+1),C6,cc6,rob(6),zeros(2,N+1),zeros(2,N+1),zeros(2,N+1),eg,eg,eg,M,A,B,ulim(6),t6,g6,j6,N,gamma,gurobi_iter,E(end),Xtight);
        [uk7{k}, xk7{k}, elapsedTime7{k}] = compute_10_agents_STL_two_edges(zeros(2,N),k,x07,zeros(2,N+1),C7,cc7,rob(7),zeros(2,N+1),zeros(2,N+1),eg,eg,M,A,B,ulim(7),t7,g7,j7,N,gamma,gurobi_iter,E(end),Xtight);
        [uk8{k}, xk8{k}, elapsedTime8{k}] = compute_10_agents_STL_three_edges(zeros(2,N),k,x08,zeros(2,N+1),C8,cc8,rob(8),zeros(2,N+1),zeros(2,N+1),zeros(2,N+1),eg,eg,eg,M,A,B,ulim(8),t8,g8,j8,N,gamma,gurobi_iter,E(end),Xtight);
        [uk9{k}, xk9{k}, elapsedTime9{k}] = compute_10_agents_STL_two_edges(zeros(2,N),k,x09,zeros(2,N+1),C9,cc9,rob(9),zeros(2,N+1),zeros(2,N+1),eg,eg,M,A,B,ulim(9),t9,g9,j9,N,gamma,gurobi_iter,E(end),Xtight);
        [uk10{k}, xk10{k}, elapsedTime10{k}] = compute_10_agents_STL_two_edges(zeros(2,N),k,x010,zeros(2,N+1),C10,cc10,rob(10),zeros(2,N+1),zeros(2,N+1),eg,eg,M,A,B,ulim(10),t10,g10,j10,N,gamma,gurobi_iter,E(end),Xtight);
        %
    else
        % propagate the dynamics
        % compute predicted dynamics for each agent-i
        xk1_pred{k}=compute_dynamics(xk1{k-1},uk1{k-1},vk{1},wk{1}(:,k-1),k,A,B,N);
        xk2_pred{k}=compute_dynamics(xk2{k-1},uk2{k-1},vk{2},wk{2}(:,k-1),k,A,B,N);
        xk3_pred{k}=compute_dynamics(xk3{k-1},uk3{k-1},vk{3},wk{3}(:,k-1),k,A,B,N);
        xk4_pred{k}=compute_dynamics(xk4{k-1},uk4{k-1},vk{4},wk{4}(:,k-1),k,A,B,N);
        xk5_pred{k}=compute_dynamics(xk5{k-1},uk5{k-1},vk{5},wk{5}(:,k-1),k,A,B,N);
        xk6_pred{k}=compute_dynamics(xk6{k-1},uk6{k-1},vk{6},wk{6}(:,k-1),k,A,B,N);
        xk7_pred{k}=compute_dynamics(xk7{k-1},uk7{k-1},vk{7},wk{7}(:,k-1),k,A,B,N);
        xk8_pred{k}=compute_dynamics(xk8{k-1},uk8{k-1},vk{8},wk{8}(:,k-1),k,A,B,N);
        xk9_pred{k}=compute_dynamics(xk9{k-1},uk9{k-1},vk{9},wk{9}(:,k-1),k,A,B,N);
        xk10_pred{k}=compute_dynamics(xk10{k-1},uk10{k-1},vk{10},wk{10}(:,k-1),k,A,B,N);
        % compute robustness to compare later so that we either optimize or
        [rob1{k}, rob_ind1{k}, rob_joint1{k}] = compute_robustness_three_edges(xk1_pred{k},xk2{k-1},xk3{k-1},xk5{k-1},C1,cc1,eg,eg,eg,t1,g1,j1,N);
        [rob2{k}, rob_ind2{k}, rob_joint2{k}] = compute_robustness_two_edges(xk2_pred{k},xk1{k-1},xk3{k-1},C2,cc2,eg,eg,t2,g2,j2,N);
        [rob3{k}, rob_ind3{k}, rob_joint3{k}] = compute_robustness_three_edges(xk3_pred{k},xk1{k-1},xk2{k-1},xk4{k-1},C3,cc3,eg,eg,eg,t3,g3,j3,N);
        [rob4{k}, rob_ind4{k}, rob_joint4{k}] = compute_robustness_three_edges(xk4_pred{k},xk3{k-1},xk5{k-1},xk7{k-1},C4,cc4,eg,eg,eg,t4,g4,j4,N);
        [rob5{k}, rob_ind5{k}, rob_joint5{k}] = compute_robustness_three_edges(xk5_pred{k},xk1{k-1},xk4{k-1},xk6{k-1},C5,cc5,eg,eg,eg,t5,g5,j5,N);
        [rob6{k}, rob_ind6{k}, rob_joint6{k}] = compute_robustness_three_edges(xk6_pred{k},xk5{k-1},xk8{k-1},xk9{k-1},C6,cc6,eg,eg,eg,t6,g6,j6,N);
        [rob7{k}, rob_ind7{k}, rob_joint7{k}] = compute_robustness_two_edges(xk7_pred{k},xk4{k-1},xk8{k-1},C7,cc7,eg,eg,t7,g7,j7,N);
        [rob8{k}, rob_ind8{k}, rob_joint8{k}] = compute_robustness_three_edges(xk8_pred{k},xk6{k-1},xk7{k-1},xk10{k-1},C8,cc8,eg,eg,eg,t8,g8,j8,N);
        [rob9{k}, rob_ind9{k}, rob_joint9{k}] = compute_robustness_two_edges(xk9_pred{k},xk6{k-1},xk10{k-1},C9,cc9,eg,eg,t9,g9,j9,N);
        [rob10{k}, rob_ind10{k}, rob_joint10{k}] = compute_robustness_two_edges(xk10_pred{k},xk8{k-1},xk9{k-1},C10,cc10,eg,eg,t10,g10,j10,N);
        [rob1{k} rob2{k} rob3{k} rob4{k} rob5{k} rob6{k} rob7{k} rob8{k} rob9{k} rob10{k}]
        
        % communicate ri^t and xki_pred and decide if to optimize or
        % propageate the dynamics
        % agent-1
        if rob1{k}<min([rob2{k} rob3{k} rob5{k}])
            [uk1{k}, xk1{k}, elapsedTime1{k}] = compute_10_agents_STL_three_edges(uk1{k-1},k,x01,xk1_pred{k},C1,cc1,rob(1),xk2_pred{k},xk3_pred{k},xk5_pred{k},eg,eg,eg,M,A,B,ulim(1),t1,g1,j1,N,gamma,gurobi_iter,E(end),Xtight);
            cO(1,k)=1;
        else
            uk1{k}=uk1{k-1}; xk1{k}=xk1_pred{k}; elapsedTime1{k}=0;
        end
        % agent-2
        if rob2{k}<min([rob1{k} rob3{k}])
            [uk2{k}, xk2{k}, elapsedTime2{k}] = compute_10_agents_STL_two_edges(uk2{k-1},k,x02,xk2_pred{k},C2,cc2,rob(2),xk1_pred{k},xk3_pred{k},eg,eg,M,A,B,ulim(2),t2,g2,j2,N,gamma,gurobi_iter,E(end),Xtight);
            cO(2,k)=1;
        else
            uk2{k}=uk2{k-1}; xk2{k}=xk2_pred{k}; elapsedTime2{k}=0;
        end
        % agent-3
        if rob3{k}<min([rob1{k} rob2{k} rob4{k}])
            [uk3{k}, xk3{k}, elapsedTime3{k}] = compute_10_agents_STL_three_edges(uk3{k-1},k,x03,xk3_pred{k},C3,cc3,rob(3),xk1_pred{k},xk2_pred{k},xk4_pred{k},eg,eg,eg,M,A,B,ulim(3),t3,g3,j3,N,gamma,gurobi_iter,E(end),Xtight);
            cO(3,k)=1;
        else
            uk3{k}=uk3{k-1}; xk3{k}=xk3_pred{k}; elapsedTime3{k}=0;
        end
        % agent-4
        if rob4{k}<min([rob3{k} rob5{k} rob7{k}])
            [uk4{k}, xk4{k}, elapsedTime4{k}] = compute_10_agents_STL_three_edges(uk4{k-1},k,x04,xk4_pred{k},C4,cc4,rob(4),xk3_pred{k},xk5_pred{k},xk7_pred{k},eg,eg,eg,M,A,B,ulim(4),t4,g4,j4,N,gamma,gurobi_iter,E(end),Xtight);
            cO(4,k)=1;
        else
            uk4{k}=uk4{k-1}; xk4{k}=xk4_pred{k}; elapsedTime4{k}=0;
        end
        % agent-5
        if rob5{k}<min([rob1{k} rob4{k} rob6{k}])
            [uk5{k}, xk5{k}, elapsedTime5{k}] = compute_10_agents_STL_three_edges(uk5{k-1},k,x05,xk5_pred{k},C5,cc5,rob(5),xk1_pred{k},xk4_pred{k},xk6_pred{k},eg,eg,eg,M,A,B,ulim(5),t5,g5,j5,N,gamma,gurobi_iter,E(end),Xtight);
            cO(5,k)=1;
        else
            uk5{k}=uk5{k-1}; xk5{k}=xk5_pred{k}; elapsedTime5{k}=0;
        end
        % agent-6
        if rob6{k}<min([rob5{k} rob8{k} rob9{k}])
            [uk6{k}, xk6{k}, elapsedTime6{k}] = compute_10_agents_STL_three_edges(uk6{k-1},k,x06,xk6_pred{k},C6,cc6,rob(6),xk5_pred{k},xk8_pred{k},xk9_pred{k},eg,eg,eg,M,A,B,ulim(6),t6,g6,j6,N,gamma,gurobi_iter,E(end),Xtight);
            cO(6,k)=1;
        else
            uk6{k}=uk6{k-1}; xk6{k}=xk6_pred{k}; elapsedTime6{k}=0;
        end
        % agent-7
        if rob7{k}<min([rob4{k} rob8{k}])
            [uk7{k}, xk7{k}, elapsedTime7{k}] = compute_10_agents_STL_two_edges(uk7{k-1},k,x07,xk7_pred{k},C7,cc7,rob(7),xk4_pred{k},xk8_pred{k},eg,eg,M,A,B,ulim(7),t7,g7,j7,N,gamma,gurobi_iter,E(end),Xtight);
            cO(7,k)=1;
        else
            uk7{k}=uk7{k-1}; xk7{k}=xk7_pred{k}; elapsedTime7{k}=0;
        end
        % agent-8
        if rob8{k}<min([rob6{k} rob7{k} rob10{k}])
            [uk8{k}, xk8{k}, elapsedTime8{k}] = compute_10_agents_STL_three_edges(uk8{k-1},k,x08,xk8_pred{k},C8,cc8,rob(8),xk6_pred{k},xk7_pred{k},xk10_pred{k},eg,eg,eg,M,A,B,ulim(8),t8,g8,j8,N,gamma,gurobi_iter,E(end),Xtight);
            cO(8,k)=1;
        else
            uk8{k}=uk8{k-1}; xk8{k}=xk8_pred{k}; elapsedTime8{k}=0;
        end
        % agent-9
        if rob9{k}<min([rob6{k} rob10{k}])
            [uk9{k}, xk9{k}, elapsedTime9{k}] = compute_10_agents_STL_two_edges(uk9{k-1},k,x09,xk9_pred{k},C9,cc9,rob(9),xk6_pred{k},xk10_pred{k},eg,eg,M,A,B,ulim(9),t9,g9,j9,N,gamma,gurobi_iter,E(end),Xtight);
            cO(9,k)=1;
        else
            uk9{k}=uk9{k-1}; xk9{k}=xk9_pred{k}; elapsedTime9{k}=0;
        end
        % agent-10
        if rob10{k}<min([rob8{k} rob9{k}])
            [uk10{k}, xk10{k}, elapsedTime10{k}] = compute_10_agents_STL_two_edges(uk10{k-1},k,x010,xk10_pred{k},C10,cc10,rob(10),xk8_pred{k},xk9_pred{k},eg,eg,M,A,B,ulim(10),t10,g10,j10,N,gamma,gurobi_iter,E(end),Xtight);
            cO(10,k)=1;
        else
            uk10{k}=uk10{k-1}; xk10{k}=xk10_pred{k}; elapsedTime10{k}=0;
        end
    end
        %
%     if min([rob1{k} rob2{k} rob3{k} rob4{k} rob5{k} rob6{k} rob7{k} rob8{k} rob9{k} rob10{k}])>=tol
%         save('computed_traj_1Mar24.mat','xk1','uk1','xk2','uk2','xk3','uk3','xk4','uk4','xk5','uk5','xk6','uk6','xk7','uk7','xk8','uk8','xk9','uk9','xk10','uk10');
%         break
%     end
...
figure(1); hold on;
plot(x01(1), x01(2), 'x', 'MarkerSize', 13,'linewidth',1.5, 'Color',[0 0.5 0.5])
plot(x02(1), x02(2), 'x', 'MarkerSize', 13,'linewidth',1.5, 'Color',[0 1 0])
plot(x03(1), x03(2), 'x', 'MarkerSize', 13,'linewidth',1.5, 'Color',[0 0 1])
plot(x04(1), x04(2), 'x', 'MarkerSize', 13,'linewidth',1.5, 'Color',[0 1 1])
plot(x05(1), x05(2), 'x', 'MarkerSize', 13,'linewidth',1.5, 'Color',[1 0 1])
plot(x06(1), x06(2), 'x', 'MarkerSize', 13,'linewidth',1.5, 'Color',[1 1 0])
plot(x07(1), x07(2), 'x', 'MarkerSize', 13,'linewidth',1.5, 'Color',[1 0.5 0])
plot(x08(1), x08(2), 'x', 'MarkerSize', 13,'linewidth',1.5, 'Color',[0.5 0 0.5])
plot(x09(1), x09(2), 'x', 'MarkerSize', 13,'linewidth',1.5, 'Color',[1 0.8 0.8])
plot(x010(1), x010(2), 'x', 'MarkerSize', 13,'linewidth',1.5, 'Color',[0.64 0.16 0.16])
%
% plot robustness
figure(2); hold on;
...
%% propagate the dynamics
function x0N=compute_dynamics(xk,uk,vk,wk_1,k,A,B,N)
%
x0N=zeros(2,N+1);
x0N(:,1:k-1)=xk(:,1:k-1);
x0N(:,k)=A*xk(:,k-1)+B*(uk(:,k-1)+vk(:,k-1))+wk_1;
for i=k:N
    x0N(:,i+1)=A*x0N(:,i)+B*uk(:,i);
end
